...
#### Known limitation: spectrum snapshot bandwidth

Test IQ and `plot_spectrum_snapshots.py` are built around **500 kHz** complex baseband (+/- 250 kHz keyed GDSS signal bandwidth). The script resamples for display but **does not** extend receiver noise into a wider analysis window, so the plots currently show essentially the **500 kHz signal band** only. Energy outside +/- 250 kHz is not a faithful depiction of surrounding white noise. A more ideal presentation would use a **700 kHz display span** (+/- 350 kHz): the **500 kHz transmission** in the centre, with a **continuous noise floor** in the sidebands (approximately -350 to -250 kHz and +250 to +350 kHz), illustrating how keyed GDSS sits in a wider receiver passband. That requires extending the plot pipeline (for example wideband noise from File 01b plus band-limited GDSS), not resampling the existing 500 kHz `.cf32` files alone.
