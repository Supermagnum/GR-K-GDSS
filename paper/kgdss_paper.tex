...
VHF land mobile (ITU-R P.1406--inspired): \textbf{flat} Rayleigh block fading (one positive gain per symbol, constant across chips) with 50~Hz and 200~Hz \emph{labels} mapped to a mild extra AWGN scaling (vehicular harsher). Per-chip Doppler phase rotation is \emph{not} applied: it averages destructively in the keyed statistic $\mathrm{mean}(r_i/m_i)$ and incorrectly drives BER to $\approx0.5$. SOQPSK Mode~2 uses a $+0.6$~dB effective $E_b/N_0$ offset vs.\ Mode~1. LDPC rate $1/2$ overlays use ideal SNR shifts with log-BER extrapolation past the simulated grid where needed \cite{shakeel2023,repo_qrl,chung2001}.
...
\caption{VHF-style flat block Rayleigh; Doppler labels scale noise (not per-chip phase on keyed $\mathrm{mean}(r/m)$). $N{=}256$; Modes~1--2; uncoded and LDPC-shifted keyed GDSS; $E_b/N_0$ to $+25$~dB.}
\label{fig:ber8}
\end{figure}
...
The GPL-3.0 licence and OIN membership are chosen to ensure this technology remains freely available to journalists, press workers, and humanitarian organisations operating under surveillance and cannot be proprietised or restricted by any state or commercial actor.
