...
{\small ISSN 1424-8220 (style placeholder)}\\[6pt]
{\small\textbf{Author contact:} \href{mailto:krbjhvee@online.no}{krbjhvee@online.no}}\\[6pt]
{\small\textbf{IACR Cryptology ePrint Archive:} Paper 2025/108456; \url{https://eprint.iacr.org/2025/108456}. Archive record timestamp: 21 March 2026.}\\[8pt]
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.90\textwidth]{\fig{fig2_key_derivation.png}}
...
\subsection{Session-unique sync and scheduled multi-burst cadence}
The synchronisation design uses a deterministic \emph{multi-burst} cadence rather than a single burst at session start. TX and RX derive the same ordered burst-epoch list from \texttt{sync\_timing} without extra signalling: \texttt{derive\_sync\_schedule} maps keyed uniforms through an inverse Pareto CDF (heavy-tailed inter-burst intervals), bounded by configured minimum interval and session duration \cite{repo_gdss}.

\texttt{derive\_sync\_pn\_sequence}: HMAC-SHA256(\texttt{sync\_pn}, \texttt{sync-pn-v2}$\Vert$\texttt{session\_id}$\Vert$\texttt{burst\_index}) derives per-burst material expanded by ChaCha20 to $\pm1$ chips; \texttt{burst\_index}=0 preserves backward compatibility \cite{repo_gdss}.\\
\texttt{derive\_sync\_amplitude\_scaling}: deterministic per-burst log-normal amplitude jitter from a separate keyed domain label.\\
\texttt{gaussian\_envelope}: rise/fall $\exp(-x^2/2)$ on flanks, \texttt{rise\_fraction}=0.15.
...
\captionof{figure}{Conceptual standard vs keyed sync placement.}
\end{strip}
...
\captionof{figure}{Transmit stack and keying path (after \texttt{tx\_example\_kgdss.grc}).}
\end{strip}
...
\subsection{Operational bands (reference)}
The reference flowgraph and IQ test suite use a 500~kHz complex baseband sample rate. Documented examples include ISM 433~MHz with $N{=}128$ (open-hardware LinHT-class integration \cite{linht_hw}). Section~7 uses stylised VHF and HF channel labels (ITU-R P.1406-- and STANAG~4539--inspired abstractions \cite{itu1406,stanag4539}), not a full regulatory or deployment survey. Band choice, power limits, and licensing remain the operator's responsibility.
...
\subsection{Spectral observability (synthetic IQ)}
Passive detection is not only a first-order amplitude distribution: a handheld spectrum display shows \emph{where} energy concentrates in frequency. The repository \texttt{tests/plot\_spectrum\_snapshots.py} computes Welch PSD on generated \texttt{.cf32} baseband (500~kHz complex), resamples to a 600~kHz display span, and applies a Gaussian time-domain window so band edges roll off smoothly (not a calibrated laboratory measurement). Figure~\ref{fig:spectrum_security} contrasts \textbf{unkeyed} standard GDSS merged into a realistic receiver-like noise floor (Files~01b+01c) with the \textbf{keyed} case (01b+01d): the former shows a prominent central feature (sync-burst / carrier-related leakage in this model) tens of decibels above the edges, whereas the latter remains featureless apart from a mild bandpass hump. The lower row shows the same construction on plain Gaussian noise (01) and keyed transmit (03) alone (flat, mutually similar) versus unkeyed GDSS with an embedded sync burst (09+06), where a narrow peak stands out. Figure~\ref{fig:spectrum_real_hw} (when File~08 is available) uses a Blackman--Harris single-FFT view of \emph{recorded} receiver noise so the plot matches typical SDR GUIs: residual DC and IQ-imbalance terms appear at the measured noise floor level.

\begin{figure*}[t]
\centering
\begin{subfigure}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth]{\fig{fig_spectrum_realistic_plus_std.png}}
\subcaption{Realistic noise (01b) plus unkeyed GDSS (01c). Strong central feature vs band edges.}
\label{fig:spec_real_std}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth]{\fig{fig_spectrum_realistic_plus_keyed.png}}
\subcaption{Same realistic noise (01b) plus keyed GDSS (01d). No discrete peak; gentle centre hump.}
\label{fig:spec_real_keyed}
\end{subfigure}
...
\centering
\includegraphics[width=\linewidth]{\fig{fig_spectrum_baseline.png}}
\subcaption{Gaussian baseline (01).}
\label{fig:spec_base}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.31\textwidth}
\centering
\includegraphics[width=\linewidth]{\fig{fig_spectrum_keyed_gdss.png}}
\subcaption{Keyed transmit (03).}
\label{fig:spec_keyed}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.31\textwidth}
\centering
\includegraphics[width=\linewidth]{\fig{fig_spectrum_standard_gdss.png}}
\subcaption{Unkeyed GDSS with embedded sync burst (09+06). Narrow central peak.}
\label{fig:spec_std_sync}
\end{subfigure}
\caption{Spectra from \texttt{plot\_spectrum\_snapshots.py} (600~kHz display; synthetic IQ unless noted). Unkeyed standard GDSS exposes structure at DC that a keyed session does not.}
\label{fig:spectrum_security}
\end{figure*}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{\fig{fig_spectrum_real_noise.png}}
\caption{Recorded receiver noise (File~08 when present; RTL-SDR-class IQ). Blackman--Harris FFT view; DC and IQ-imbalance terms near 0~Hz at $\approx -119$~dB in representative captures---physical receiver behaviour, not simulation.}
\label{fig:spectrum_real_hw}
\end{figure}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{\fig{fig_iq_psd_row_vs_standard.png}}
\caption{Welch PSD (500~kHz, repository \texttt{plot\_iq\_comparison.py}): row~2 of \texttt{iq\_comparison\_vs\_standard.png}, Gaussian baseline (File~01), keyed GDSS (03), and standard GDSS (09). At native sample rate the unkeyed trace shows elevated low-frequency energy versus the other two.}
\label{fig:iqpsd}
\end{figure*}
...
\begin{tabular}{@{}p{0.28\columnwidth}p{0.34\columnwidth}p{0.36\columnwidth}@{}}
\toprule
...
\begin{table*}[t]
\centering
\caption{gr-k-gdss pytest modules (snapshot: 42 passed, 1 skipped).}
\scriptsize
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}ll@{}}
\toprule
...
\texttt{test\_p372\_receiver\_profile.py} (3 tests: loader, expected profile, calibration) & PASSED \\
...
\texttt{test\_t2\_sync\_burst.py} (16 tests: PN, schedule, per-burst PN, envelope, masking) & PASSED \\
...
% Full-width BER figures overlap twocolumn body text if kept as figure*[t]; use onecolumn here.
\onecolumn
...
\label{eq:dsss}
\end{equation}
Curves in Figs.~\ref{fig:ber7}--\ref{fig:ber10} are produced by \texttt{paper/ber\_simulation.py} (NumPy/SciPy), aligned with the GNU Radio stack conceptually but without invoking the SDR runtime. Spreading factors $N\in\{64,128,256\}$ match the implementation. The SNR axis is $E_b/N_0$ in dB from $-20$ to $+25$ in 1~dB steps (upper end extended so keyed \texttt{mean}$(r/m)$ curves are not mistaken for a stuck receiver near BER $=0.5$). Unless overridden by the environment variable \texttt{BER\_MC\_NUM\_BITS}, each plotted point uses $10^6$ simulated bits per receiver condition.
...
VHF land mobile (ITU-R P.1406--inspired): \textbf{flat} Rayleigh block fading (one gain per symbol, constant across chips) with 50~Hz and 200~Hz \emph{labels} as extra AWGN scaling (vehicular harsher). Per-chip Doppler phase rotation is \emph{not} applied: it destructively averages in the keyed statistic $\mathrm{mean}(r_i/m_i)$ and incorrectly drives BER to $\approx0.5$. Fig.~\ref{fig:ber8} uses an extended $E_b/N_0$ grid to $+40$~dB (other figures use $+25$~dB); physical interpretation of the resulting curves is in Section~7.3. SOQPSK Mode~2 uses a $+0.6$~dB effective $E_b/N_0$ offset vs.\ Mode~1. LDPC rate $1/2$ overlays use ideal SNR shifts with log-BER extrapolation where needed \cite{shakeel2023,repo_qrl,chung2001}.
...
The curves are \emph{not} intended to match the sharp BER waterfall of Fig.~\ref{fig:ber7} (AWGN). Under \textbf{flat block Rayleigh} fading the same random amplitude applies to every chip in a symbol, so despreading does \emph{not} average over independent fades: a deep fade suppresses the whole block, and increasing $E_b/N_0$ improves BER gradually (shallower fades become recoverable) rather than opening an AWGN-like cliff. That behaviour is physically consistent with the model; it is not a Monte Carlo error. Mode~2 is approximated by a small effective SNR offset ($+0.6$~dB), reflecting modest multicarrier diversity when not all carriers share the same deep fade. Ideal LDPC overlays assume AWGN-like error statistics; without interleaving across independent fades, coding gain against burst errors from entire faded blocks remains limited. Dense urban VHF with a single carrier is therefore a challenging regime for any wideband spread-spectrum link under this abstraction.

The irreducible BER floor apparent in Fig.~\ref{fig:ber8} arises from the flat block Rayleigh model (no chip-level or frequency diversity in the simulator). \textbf{Future simulation work:} per-chip independent Rayleigh fading where coherence time is shorter than a symbol, and/or frequency-selective tapped-delay-line fading across the spread bandwidth, would expose diversity gain from $N$-fold despreading and from spreading chips---as in operational wideband systems.
...
\subsection{Figure set and scope}
Figs.~\ref{fig:ber7}--\ref{fig:ber10} are complementary: AWGN (Fig.~\ref{fig:ber7}) isolates mask-processing loss; VHF (Fig.~\ref{fig:ber8}) illustrates limits of the flat block Rayleigh model (Section~7.3); HF (Fig.~\ref{fig:ber9}) uses tapped-delay stylisations; LDPC (Fig.~\ref{fig:ber10}) applies ideal SNR shifts to the keyed uncoded curve. None substitute for calibrated over-the-air measurements or full modem chains.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{\fig{fig7_awgn_ber.png}}
\caption{AWGN: DSSS \eqref{eq:dsss} vs Monte Carlo standard and keyed GDSS ($N=64,128,256$). Keyed GDSS curves use a darker orange and heavier stroke than standard GDSS for contrast in colour print and greyscale.}
\label{fig:ber7}
\end{figure}
...
\caption{VHF-style flat block Rayleigh; Doppler labels scale noise (not per-chip phase on keyed $\mathrm{mean}(r/m)$). $N{=}256$; Modes~1--2; uncoded and ideal LDPC-shifted keyed GDSS; $E_b/N_0$ to $+40$~dB (this figure). Behaviour differs from AWGN Fig.~\ref{fig:ber7}: see Section~7.3. The irreducible BER floor arises from the flat block Rayleigh model; per-chip independent fading or frequency-selective models would show diversity gain from $N{=}256$ despreading and are identified as future simulation work.}
\label{fig:ber8}
\end{figure}
...
\caption{Keyed GDSS on AWGN: uncoded Monte Carlo to $+25$~dB $E_b/N_0$ (mask division yields gradual slope); ideal LDPC rate $1/2$ shifts with 576- and 1152-bit gain anchors ($N{=}256$).}
\label{fig:ber10}
\end{figure}
...
\caption{I-channel amplitude histograms: Gaussian noise baseline (01), keyed spread transmission (03), wrong-key despread (05). \textbf{Keyed chips} follow the transmit model $s\cdot m_I$ with BPSK $s\in\{\pm1\}$ and Gaussian mask $m_I$ (same construction as \texttt{tests/generate\_iq\_test\_files.py}), which is marginally Gaussian and matches the baseline; the figure uses subsampled \texttt{tests/iq\_files/*.cf32} when those files are present, else the same statistics from synthetic data. \textbf{Not shown:} unrelated products of normals, which would wrongly imply a narrow spike.}
\end{figure}
...
Formal indistinguishability games for ChaCha20-derived masks, complete OTA validation with Nitrokey-backed keys, and integration of ephemeral ECDH for forward secrecy remain outstanding \cite{repo_gdss}. Beyond the implemented multi-burst cadence, the current engineering roadmap prioritises (i) real-time receiver-side noise-floor estimation from rolling PSD statistics, (ii) SQLite persistence of noise-floor and impulse-rate history for warm starts and local adaptation, and (iii) optional GPS-assisted dual-site exchange of those histories via out-of-band channels. ITU-R P.372-17 is the normative reference for atmospheric radio noise \cite{p372}; the present codebase exposes only lightweight integration hooks (\texttt{p372\_baseline.py}, \texttt{p372\_receiver\_profile.py})---a static burst-parameter source and a synthetic PSD prior with median calibration, not \S3.1.1 instantaneous or \S3.1.2 statistical brightness-temperature prediction. A practical receiver architecture is to compare per-bin PSD from GNU Radio receiver blocks against that prior while live estimation (roadmap Priority~2) supersedes the cold-start fallback.

\subsection{Threat model and operational context}
Journalists, press photographers, humanitarian workers, and human rights monitors in conflict zones or under authoritarian surveillance face a possible threat from state-level SIGINT assets. Encryption conceals content but does not conceal the fact of transmission or the transmitter's location. Keyed GDSS below the noise floor addresses precisely this gap.

\subsection{LinHT hardware platform}
LinHT is the most operationally practical hardware available today. Built into a Retevis C62 case, it is visually indistinguishable from a standard commercial handheld---carrying it through a checkpoint does not immediately identify SDR equipment. The USB-C port accepts a Nitrokey~3 for hardware-protected key operations. The device runs full Linux: \texttt{gr-k-gdss} flowgraphs, GnuPG, and the session key derivation scripts from Section~4.3 run natively. The 32~GB eMMC stores keying material and flowgraphs. At 433~MHz with $N{=}128$ and 500~kHz spread, processing gain is 21~dB, placing the signal 21~dB below the noise floor of any narrowband ISM-band receiver. Combined with keyed Gaussian masking, session-unique sync bursts, and ChaCha20-Poly1305 payload encryption, the LinHT-based keyed GDSS stack provides covertness substantially beyond any commercially available encrypted radio in a comparable form factor.

\subsubsection{Stack alignment}
The GR-K-GDSS stack uses only components already recommended by the press-freedom community: GnuPG \cite{gnupg,fpf,eff}, Linux, and GNU Radio \cite{gnu_radio}. Nitrokey is commercially available and recommended by privacy advocates. LinHT hardware files are open-source \cite{linht_hw}. No export-controlled components or proprietary firmware are required---the stack is auditable end-to-end.

\subsubsection{Deployment limitations (LinHT)}
...
Clone \cite{repo_gdss}; install GNU Radio 3.10+, libsodium, gr-linux-crypto, gr-qradiolink as dependencies; build with CMake. Generate IQ fixtures: \texttt{python3 tests/generate\_iq\_test\_files.py}. Analyse: \texttt{python3 tests/analyse\_iq\_files.py}. Spectrum snapshots (Section~5) and Welch comparison plots: \texttt{python3 tests/plot\_spectrum\_snapshots.py}; \texttt{python3 tests/plot\_iq\_comparison.py} (requires Files~01, 03, 09, 12, 13 for \texttt{iq\_comparison\_vs\_standard.png}). BER Monte Carlo data: \texttt{cd paper \&\& python3 ber\_simulation.py} (optional: \texttt{BER\_MC\_NUM\_BITS=50000} for a faster dry run). Figures: \texttt{cd paper \&\& python3 gen\_figures.py} (loads \texttt{paper/figures/ber\_mc\_results.npz} for Figs.~\ref{fig:ber7}--\ref{fig:ber10}; copies staged IQ PNGs from \texttt{tests/iq\_files/} into \texttt{paper/figures/} with placeholders if missing). PDF: \texttt{pdflatex kgdss\_paper.tex} (twice).
...
The authors intend \textbf{Open Invention Network} (OIN) registration for the project as a defensive measure aligned with Linux-related ecosystems; if this preprint appears before registration is complete, treat OIN status as \emph{in progress}, not a completed filing.
...
LinHT hardware is licensed under CC~BY-NC-SA~4.0 \cite{cc_by_nc_sa_40}. \texttt{gr-k-gdss}, \texttt{gr-linux-crypto}, and \texttt{gr-qradiolink} are GPL-3.0-or-later. The combined stack is fully open-source, auditable, and freely reproducible.
...
\bibitem{bash2015} B. A. Bash \textit{et al.}, ``Limits of Reliable Communication with Low Probability of Detection on AWGN Channels,'' \textit{IEEE Commun. Mag.}, vol.~53, no.~4, pp.~26--32, Apr. 2015.
\bibitem{bash2013} B. A. Bash \textit{et al.}, ``Square Root Law for Communication with Low Probability of Detection on AWGN Channels,'' in \textit{Proc. IEEE ISIT}, 2013, pp.~448--452.
\bibitem{turner1991} L. F. Turner, ``The Vulnerability of Laser-Based Covert Communication Systems to High-Power Noise Jamming,'' in \textit{Proc. IEEE NAECON}, 1991, pp.~714--718.
\bibitem{gardner1992} W. A. Gardner and C. M. Spooner, ``Signal interception: performance advantages of cyclic-feature detectors,'' \textit{IEEE Trans. Commun.}, vol.~40, no.~1, pp.~149--159, Jan. 1992.
...
\bibitem{p372} ITU-R P.372-17, ``Radio noise,'' International Telecommunication Union, 2024.
...
\bibitem{gnupg} GNU Privacy Guard (GnuPG), \url{https://gnupg.org/}
\bibitem{fpf} Freedom of the Press Foundation (digital security for journalists), \url{https://freedom.press/}
\bibitem{eff} Electronic Frontier Foundation (digital rights), \url{https://www.eff.org/}
\bibitem{gnu_radio} GNU Radio project, \url{https://www.gnuradio.org/}
\bibitem{linht_hw} LinHT open hardware (Retevis C62-class integration; design files and Gerbers published as open hardware; consult current project documentation for repository URLs).
\bibitem{cc_by_nc_sa_40} Creative Commons, ``CC BY-NC-SA 4.0 Attribution-NonCommercial-ShareAlike 4.0 International,'' \url{https://creativecommons.org/licenses/by-nc-sa/4.0/}
\bibitem{eprint_kgdss_2025} \textit{Cryptographically Keyed Gaussian-Distributed Spread-Spectrum for Enhanced Covert Communications: Design, Implementation, and Simulated Performance in ITU Channel Models.} IACR Cryptology ePrint Archive, Report 2025/108456 (archive record 21 March 2026). \url{https://eprint.iacr.org/2025/108456}
\end{thebibliography}
